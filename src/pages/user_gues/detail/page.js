  ...
  const ActionBtn = props => {
    ...
    const increment = e => {
      if (totalTicket >= maximum_purchase_ticket) {
        ...
        totalPriceAll =
          parseInt(e.target.getAttribute("data-price")) + totalPriceAll;
    ...
    const reduction = e => {
      if (values > 0) {
        ...
        totalTicket = totalTicket - 1;
        totalPriceAll =
          totalPriceAll - parseInt(e.target.getAttribute("data-price"));
        totalPriceText.innerHTML = new Intl.NumberFormat().format(
          totalPriceAll
        );
      }
      ...
      return (
        <>
          {!props.ticketSold ? (
            ...
            <p className="text-red-500 font-semibold uppercase">Sold</p>
          )}
        </>
      );
    };
    ...
    if (totalTicket <= maximum_purchase_ticket) {
      for (let count = 0; count < total_ticket.length; count++) {
        if (total_ticket[count].value > 0) {
          ...
          DataLocalStorage["fields"]["qty_" + ticket_id_qty[count].value] =
            parseInt(document.getElementById("total_ticket_" + count).value);
          ...
          <div className="grid grid-cols-5 max-md:grid-cols-1 max-md:gap-0 gap-3 mt-5">
            ...
            <div className="col-span-2 mx-4 shadow-md p-5 overflow-hidden h-full max-md:m-0">
              <div className="space-y-4 flex flex-col align-baseline justify-between h-full bg-white ">
                <div>
                  <div>
                    <h1 className="font-semibold text-xl">{name_event}</h1>
                  </div>
                  <div className="space-y-2 mt-6">
                    ...
                    <p className="text-base">
                      Time : {start_time} {end_time ? ` - ${end_time} WIB` : ""}{" "}
                    </p>
                ...
                <div>
                  {" "}
                  <p className="font-semibold text-sm">Diselenggarakan Oleh</p>
                    ...
                    <h2 id="accordion-open-heading-1">
                      <button
                        ...
                        <span className="flex items-center">
                          <h2 className="font-semibold">Tiket</h2>
                        </span>
                        <svg
                          data-accordion-icon
                    ...
                    {ticket.map((value, key) => (
                      <div
                        className={`bg-gray-50 border my-3 px-3 py-2 rounded mx-4 ${
                          value.disable_qty_input !== 0 ? "cursor-pointer" : ""
                        } `}
                        key={key}>
                        <div className="space-y-5">
                          ...
                          <div>
                            <p className="text-sm mt-5 line-clamp-none">
                              Paket hemat 4 orang dalam 1 mobil + PHOTOBOOTH.
                              Pukul 14.00 - 17.00. Apabila penumpang lebih,
                              tiket tambahan dapat di beli di tempat
                            </p>
                          </div>
                          <div>
                            {" "}
                            {value.remaining_qty_in_percentage === 1 &&
                            value.remaining_qty > 0 ? (
                              <>
                                <span className="text-green-600 font-semibold bg-green-300 px-3 py-2 rounded">
                                  Sisa Tiket {value.remaining_qty} %{" "}
                                </span>
                              </>
                            ) : value.remaining_qty <= 5 &&
                              value.remaining_qty > 0 ? (
                              <>
                                <span className="text-red-500 font-semibold bg-red-200 px-3 py-2 rounded">
                                  Sisa {value.remaining_qty} Tiket{" "}
                                </span>
                              </>
                            ) : (
                              ""
                        ...
                        <div className="flex justify-between items-center mt-12">
                          <div>
                            <p className="mt-3 max-md:m-0 font-semibold">
                              {value.price !== "0" ? value.price : "GRATIS"}
                            </p>
                          </div>

                          <ActionBtn
                            dataPrice={value.price_value}
                            dataKey={key}
                            dataId={value.code}
                            ticketSold={value.sold_out}
                            DisableQtyInput={value.disable_qty_input}
                            dataName={value.name}
                            dataIdTicket={value.id}
                            dataRemaindingInPercentage={
                              value.remaining_qty_in_percentage
                            }
                            dataRemaindingQty={value.remaining_qty}
                          />
                        </div>
                      </div>
                    ))}
            ...
            <div className="col-span-2 mx-4 max-md:m-0 ">
              <div className="p-5 space-y-5 mt-5 ">
                <div className="flex items-center gap-5">
                  ...
                  <p className="text-base">
                    Kamu belum memilih tiket. Silakan pilih lebih dulu di tab
                    menu TIKET.
                  </p>
                </div>
                <div>
                ...
                <div className="flex justify-between">
                  ...
                  <div className="flex gap-3 font-semibold">
                    <p>IDR</p>
                    <p id="totalPrice">
                      {new Intl.NumberFormat().format(totalPriceAll)}
                    </p>
                  </div>
                </div>
                <div className="mt-4">

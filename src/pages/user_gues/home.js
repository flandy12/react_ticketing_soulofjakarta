...
import {CallApi, GetKey, GetUrl} from '../../helper/Api';
...
const HomePage = () => {
    ...
    const API = () => {
        CallApi('event-dashboard', {
            ...
            headers: {
                ...
                if(res.success === true) {
                setEvent(res.results.testing && res.results.testing.length > 0 ?  res.results.testing : res.results.events);
                    ...
                    <div className="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2  flex-wrap justify-start  gap-5 mx-5">
                        {event.map((value,key) => (
                            ...
                            <Link to={`/detail/${value.id}`} className="" key={key}>
                                ...
                                <img className="rounded-t-lg h-[139px] object-cover w-auto " src={`${value.main_img}`} alt="img-event"  fetchpriority="high" loading="lazy" />
                            

                                <div className="space-y-5">
                                    <div>
                                        <h5 className="text-lg font-bold tracking-tight text-gray-800  whitespace-normal  overflow-hidden h-[60px] max-h-full w-full line-clamp-2 " title={value.name}>{value.name}</h5>
                                    </div>
                                    <div className="flex flex-col gap-2">
                                        <div className="flex gap-5">
                                            <img src="/images/icon/icon_calendar_.svg" alt="calendar-asset"/>
                                            <span>{value.start_date} - {value.end_date}</span>
                                        </div>
                                        <div className="flex gap-5">
                                            <img src="/images/icon/icon_clock_.svg" alt="clock-asset"/>
                                            <span>{value.start_time} - {value.end_time}</span>
                                        </div>

                                        <div className={`${value.venue === null ?'hidden' : 'flex'} gap-5`}>
                                            <img src="/images/icon/location_.svg" alt="location-asset"/>
                                    ...
                                    <div className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 max-w-[120px]">
                                        <span className="text-center">IDR {value.from_price}</span>
                                    </div>
                                </div>
    ...
    const SkeletonCardEvent = () => {
        ...
        for (let index = 0; index < 3; index++) {
            element.push(
                <div>
                    <div role="status" className="space-y-8 animate-pulse rtl:space-x-reverse md:flex md:items-center flex-col">
    ...
    const HomePageWrapper = () => {
        return (
            <>
                <LoadingAnimation/>
                <Navbar/>
                <div className=""> {isLoading ? <SkeletonCardEvent/> : <HomeContainer/> }</div>
            </>
